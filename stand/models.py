...
class StatusExecution:
    ...
    def values():
        return [
            n
            for n in list(StatusExecution.__dict__.keys())
...
class ResultType:
    ...
    def values():
        return [
            n
            for n in list(ResultType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class ClusterType:
    ...
    def values():
        return [
            n
            for n in list(ClusterType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class JobType:
    ...
    def values():
        return [
            n
            for n in list(JobType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class ClusterPermission:
    ...
    def values():
        return [
            n
            for n in list(ClusterPermission.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class TriggerType:
    ...
    def values():
        return [
            n
            for n in list(TriggerType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class Job(db.Model):
    ...
    cluster_id = Column(
        Integer,
        ForeignKey("cluster.id", name="fk_job_cluster_id"),
        nullable=False,
        index=True,
    )
    cluster = relationship(
        "Cluster", overlaps="cluster", foreign_keys=[cluster_id]
    )
    pipeline_step_run_id = Column(
        Integer,
        ForeignKey("pipeline_step_run.id", name="fk_job_pipeline_step_run_id"),
        index=True,
    )
    pipeline_step_run = relationship(
        "PipelineStepRun", overlaps="job", foreign_keys=[pipeline_step_run_id]
    )
    pipeline_run_id = Column(
        Integer,
        ForeignKey("pipeline_run.id", name="fk_job_pipeline_run_id"),
        index=True,
    )
    pipeline_run = relationship(
        "PipelineRun", overlaps="job", foreign_keys=[pipeline_run_id]
    )
    steps = relationship("JobStep", cascade="all, delete-orphan")
...
class JobStep(db.Model):
    ...
    job_id = Column(
        Integer,
        ForeignKey("job.id", name="fk_job_step_job_id"),
        nullable=False,
        index=True,
    )
    job = relationship(
        "Job", overlaps="steps", foreign_keys=[job_id], back_populates="steps"
    )
    logs = relationship("JobStepLog", cascade="all, delete-orphan")
...
class JobStepLog(db.Model):
    ...
    step_id = Column(
        Integer,
        ForeignKey("job_step.id", name="fk_job_step_log_step_id"),
        nullable=False,
        index=True,
    )
    step = relationship(
        "JobStep", overlaps="logs", foreign_keys=[step_id], back_populates="logs"
    )
...
class JobResult(db.Model):
    ...
    job_id = Column(
        Integer,
        ForeignKey("job.id", name="fk_job_result_job_id"),
        nullable=False,
        index=True,
    )
    job = relationship(
...
class Cluster(db.Model):
    ...
    executors = Column(Integer, default=1, nullable=False)
    executor_cores = Column(Integer, default=1, nullable=False)
    ...
    platforms = relationship("ClusterPlatform", cascade="all, delete-orphan")
...
class ClusterFlavor(db.Model):
    ...
    cluster_id = Column(
        Integer,
        ForeignKey("cluster.id", name="fk_cluster_flavor_cluster_id"),
        nullable=False,
        index=True,
    )
    cluster = relationship(
...
class ClusterPlatform(db.Model):
    ...
    # Fields
    id = Column(Integer, primary_key=True)
    platform_id = Column(Integer, default=1, nullable=False)

    # Associations
    cluster_id = Column(
        Integer,
        ForeignKey("cluster.id", name="fk_cluster_platform_cluster_id"),
        nullable=False,
        index=True,
    )
    cluster = relationship(
...
class ClusterAccess(db.Model):
    ...
    cluster_id = Column(
        Integer,
        ForeignKey("cluster.id", name="fk_cluster_access_cluster_id"),
        nullable=False,
        index=True,
    )
    cluster = relationship(
...
class ClusterConfiguration(db.Model):
    ...
    cluster_id = Column(
        Integer,
        ForeignKey("cluster.id", name="fk_cluster_configuration_cluster_id"),
        nullable=False,
        index=True,
    )
    cluster = relationship(
...
class Room(db.Model):
    ...
    consumers = Column(Integer, default=0, nullable=False)
...
class RoomParticipant(db.Model):
    ...
    room_id = Column(
        Integer,
        ForeignKey("room.id", name="fk_room_participant_room_id"),
        nullable=False,
        index=True,
    )
    room = relationship(
...
class PipelineStepRun(db.Model):
    ...
    created = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
    updated = Column(
        DateTime, nullable=False, index=True, onupdate=datetime.datetime.utcnow
    )
    ...
    pipeline_run_id = Column(
        Integer,
        ForeignKey(
            "pipeline_run.id", name="fk_pipeline_step_run_pipeline_run_id"
        ),
        nullable=False,
        index=True,
    )
