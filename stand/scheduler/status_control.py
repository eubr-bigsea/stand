...
from stand.models import (Job, PipelineRun, PipelineStepRun, StatusExecution,Pipeline,PipelineStep)
...
def propagate_job_status(run:PipelineRun):
    ...
    if(active_step_run is not None):
        ...
        if(run.status==StatusExecution.RUNNING and
            latest_job_status== StatusExecution.COMPLETED):
                update_pipeline_run_status(run,StatusExecution.WAITING)
                update_pipeline_step_run_status(active_step_run,
                                                         StatusExecution.COMPLETED)
               
        #run was waiting and a step was triggered
        elif(run.status==StatusExecution.WAITING and
            latest_job_status==StatusExecution.RUNNING):
               update_pipeline_run_status(run,StatusExecution.RUNNING)
               
        # error in job during run
        elif(run.status == StatusExecution.RUNNING and
             latest_job_status == StatusExecution.ERROR):
