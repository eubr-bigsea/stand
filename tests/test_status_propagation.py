...
def mocked_functions():
    with patch("stand.scheduler.status_control.get_latest_pipeline_step_run") as mocked_get_latest_pipeline_step_run, \
         patch("stand.scheduler.status_control.get_latest_job_from_pipeline_step_run") as mocked_get_latest_job_from_pipeline_step_run, \
         patch("stand.scheduler.status_control.update_pipeline_run_status") as mocked_update_pipeline_run_status, \
         patch("stand.scheduler.status_control.update_pipeline_step_run_status") as mocked_update_pipeline_step_run_status:
        
        yield (mocked_get_latest_pipeline_step_run,
...
def test_completed_job_status_propagation(mocked_functions):
    ...
    mocked_get_latest_job_from_pipeline_step_run, \
    mocked_update_pipeline_run_status, \
    mocked_update_pipeline_step_run_status = mocked_functions
    latest_job =Job(id=1, status = StatusExecution.COMPLETED)
    
    step_run =PipelineStepRun(id="1", 
    ...
    mocked_update_pipeline_run_status.assert_called_with(pipeline_run,
                                                              StatusExecution.WAITING)
    mocked_update_pipeline_step_run_status(pipeline_run,
